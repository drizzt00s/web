...
//for ajax get
exports.ajaxQueryPost=function(req,res){
       var getAjaxData=req.body;
       //虽然放在http体里面传来的数据是字符串，但是这里的返回值值还是给对象，是因为express已经帮忙处理了
       var userSentTime=new Date().toLocaleString();
       var userSentTimeArray=userSentTime.split(" ").pop();
       var sentWeek=userSentTimeArray[0];
       var sentMonth=userSentTimeArray[1];
       var sentDay=userSentTimeArray[2];
       var sentYear=userSentTimeArray[3];
       var sentHour=userSentTimeArray[4];
       if(sentWeek=="Mon"){
           sentWeek="星期一"
       }
       else if(sentWeek==""){
       }
       // getAjaxData["whenSent"]=new Date().toLocaleString();
       //添加时间
       var getAjaxDataString=JSON.stringify(getAjaxData);//string
       var formWhom=getAjaxData["from"];//帐号
       var whoSent=getAjaxData["fromFalseName"];//昵称
       var dataBody=getAjaxData["data"];
       var dataTo=getAjaxData["to"];
       var whenSent=getAjaxData["whenSent"];
       var Client =require("mysql").Client;
       var client =new Client();
       client.user="root";
       client.password="5225541a";
       ...
       client.query(queryString,function(error,result){
             ...
             if(!getMsgAsyn){
                ...
                // storeMsgJson['con'].push(getAjaxDataString);
		 ...
		 var queryStringSentMsg="insert into message (username,sentmsg,towhom,time) values('"+formWhom+"'"+","+"'"+dataBody+"'"+","+"'"+dataTo+"'"+","+"'"+whenSent+"')";
...
exports.signOut=function(req,res){
 ...
 if(checkSession&&checkSession!=""){
	...
	/* added on 2013-20-29*/
	   thisLoginFlag=0;
	/* added on 2013-20-29*/
	   res.redirect("/WebstormProjects/web/views/index.ejs");
...
exports.editProfile=function(req,res){
   ...
   var client=prepareDb();
   //var queryString="select * from d where account='1'";
   ...
   //查出所有用户的图片
     var profileList=eval('('+r[0]["avatar"]+')');
	  if(profileList){
		 ...
		 res.render('./editProfile',{title:'edit profile', profiles:JSON.stringify(profileList)});
...
/*
exports.fetchDailyMatch=function(req,res){

  console.log("fetchDailyMatch!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
