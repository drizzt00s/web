...
exports.sendMsg = function(req, res){
	...
	client.query(queryString, function(error, d){
		...
		if(!(d[0].avatar)){
			...
			var avatarsLength = eval("(" + d[0].avatar + ")").length;  
...
exports.msgsent = function(req,res){
	...
	var msgType = getAjaxData['msgType'];
	...
	client.query(queryString, function(error, d){
		...
		if((d[0].profile)){
			...
			var profile = profileWithoutPath;//有路径的完整profile
		} else {
		...
		client.query(queryString,function(error,result){
			...
			//msgAsyn字段中无值
				...
				console.log(currentMsgString);
				var currentMsgStringToJson = eval('(' + currentMsgString +' )');
				//var currentMsgStringToJson = JSON.parse(currentMsgString);
		...
		var queryStringSentMsg = "insert into message (username,msgTag,targetProfile,sentmsg,msgTimestamp,towhom,selfProfile,selfFasleName,isMsgNew,toWhomAccount,msgType,time) values('" +
...
exports.changeNewMsgToOld = function(req,res){
    ...
    var msgTag = req.body.msgTag;
    var msgTimestamp = req.body.msgTimestamp;
    var targetUsername = req.body.targetUsername;//发信息的人的用户名
    ...
    client.query(queryString, function(error, d){
        ...
        for(var i = 0; i < msgAsynArray.length; i++ ){
            if(msgAsynArray[i]['msgTag'] === msgTag && msgAsynArray[i]['msgTimestamp'] == msgTimestamp){
        ...
        client.query(queryString, function(error, d){
            ...
            var queryString = "update message set isMsgNew='" + 0 + "'" + 
...
exports.buildDataTree = function(req, res){
	var whoSentAccount = req.body.whoSentAccount;
	var toWhomAccount = req.body.toWhomAccount;
	var msgTag = req.body.msgTag;
	var client = utility.prepareDb();
	var queryString = "select msgAsyn from d where account = '" + whoSentAccount +"'";
	var msgFound = [];
...
exports.outboxDetail = function(req,res){
	...
	var toWhomAccount = req.query.toWhomAccount; //对方的用户名
	var msgTimestamp = req.query.msgTimestamp;
	var msgType = req.query.msgType;
	var isMsgNew =  req.query.isMsgNew;
	var selfAccount = req.query.self;

	var client = utility.prepareDb();
	var queryString = "select msgAsyn from d where account ='" + toWhomAccount +"'";
	var msgFound = [];
	client.query(queryString, function(e, d){
		if(e){
			throw e;
		...
		if(d){
			...
			var data = d['con'];
			for(var i = 0; i < data.length; i++){
				if(selfAccount === data[i]['from'] && msgTag === data[i]['msgTag']){
					msgFound.push(data[i]);
				}
			}
			res.render("./msg/outboxDetail.ejs",{"title":"发件箱聊天记录", "msgFound":JSON.stringify(msgFound)});
		}
	});
};

exports.allSentMsg = function(req, res){
