...
//db ready
    ...
    client.query(queryString,function cBack(error,result,fields){
        ...
        if(checkResultLength==0){
            accountErrorMsg="此用户名不存在！";
            storeQueryResult["accResult"]=accountErrorMsg;
            storeQueryResult["passResult"]=passErrorMsg;
            var storeQueryResultString=JSON.stringify(storeQueryResult);

           // console.log(storeQueryResultString);
           // console.log("!!!!!!!!!!!!!!!!!");

            res.writeHead("200",{"Content-Type":"text/plain"});
            res.write(storeQueryResultString);
            res.end();
        }
        //用户名不存在
        else{
            ...
            client.query(queryString2,function(error,result,fields){
                ...
                if(result.length==0){
                    ...
                    var storeQueryResultString=JSON.stringify(storeQueryResult);
                    res.write(storeQueryResultString);
				            //res.render("./index.ejs",{title:"home",title2:"mismatch!",accErrMsg:"",passwordErrMsg:"",isSignUp:1,userName:userFalseName,lastRequestType:"post"});
                    res.end();
...
//get register data
    ...
    client.query(queryString,function cBack(error,results,fields){
        ...
        if(queryResult!=0){
            var accountErrorMsgV="该用户名已经被注册！"
            var passwordErrorMsgV="";
            res.render("./register.ejs",{
                title:"register",
                accountErrorMsg:accountErrorMsgV,
                passwordErrorMsg:passwordErrorMsgV,
                successMsg:"",
                regHtml1:registerHtml1,
                regHtml2:registerHtml2,
                userAccount:account,
                userPass:pass,
                userCheckPass:checkPass,
                userAddressProvince:userAddressProvince,
                userAddressCity:userAddressCity,
                userFalseName:falseName,
                //userGender:gender,
                userBirthday:birthday,
                userMail:mail,
                userTrueName:trueName,
                userIndentityCard:indentityCard,
                userAddress:userAddress,
                userPostCode:postCode,
                userMobile:mobile,
                userPhone:phone,
                userWebsite:website,
                userMarriageStatus:marriageStatus,
                userEducation:education,
                userHeight:height
            });
        }
        else{
            var checkResult=_this.clientDataValidation();
            var accountErrorMsgV=checkResult["Qon"].checkAccountResult;
            var passwordErrorMsgV=checkResult["Qon"].checkPassResult;
            if(accountErrorMsgV!=""||passwordErrorMsgV!=""){
                res.render("./register.ejs",{
                    title:"register",
                    accountErrorMsg:accountErrorMsgV,
                    passwordErrorMsg:passwordErrorMsgV
 ...
 //create a db instance
    ...
    function finalSave(queryString){
        client.query(queryString,function(error){
           ...
           //res.render("./register.ejs",{title:"register",accountErrorMsg:"",passwordErrorMsg:"",successMsg:"注册成功！",regHtml1:registerHtml1,regHtml2:registerHtml2});
           //设置session
...
User.prototype.clientDataValidation=function(){
    var storeCheckResult={"Qon":{}};
    var accountErrorMsgV="";
    var passwordErrorMsgV="";
    var account=this.username;
    ...
    if(accountLength<=6){
        accountErrorMsgV="用户名长度不得小于6位!"
    }
    if(password!=checkPass){
        passwordErrorMsgV="二次输入的密码不一致！"
    }
    storeCheckResult["Qon"].checkAccountResult=accountErrorMsgV;
    storeCheckResult["Qon"].checkPassResult=passwordErrorMsgV;
